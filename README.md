...
- `-deviceDisconnectGrace` (int): время в секундах, в течение которого временно исчезнувшее USB-устройство остаётся в списке. Нужен для плат, которые кратко пропадают из USB при перезагрузке; значение `0` отключает задержку (по-умолчанию 1).
...
| get-firmware-next-block           |                                                                                                                                                       | Запрос от клиента на принятие следующего блока с бинарными данными, выгруженной прошивки. Если все данные были переданы клиенту, то сервер отправит сообщение типа ms-get-firmware-finish.                                                                                                                                                                                                                                                                     | клиент   |
