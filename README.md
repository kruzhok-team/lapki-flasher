...
| serial-connection-status | deviceID, isConnected (bool), comment | isConnected = true, если установлено подключение к устройству deviceID,  comment - дополнительное сообщение, содержит текст ошибки.  Автоматически отправляется сервером при изменении статуса соединения. | сервер |
