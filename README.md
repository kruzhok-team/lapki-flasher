...
| serial-connection-status | deviceID, isConnected (bool), comment | isConnected = true, если установлено подключение к устройству deviceID,  comment - дополнительное сообщение, содержит текст ошибки.  Автоматические отправляется сервером при изменении статуса соединения. | сервер |
...
| serial-sent-status       | deviceID, hasSent (bool), comment     | Сведения о последнем отправленном на deviceID сообщении: hasSent = true, если сообщение было отправлено успешно; comment - дополнительное сообщение, содержит текст ошибки.                                  | сервер |
