...
ms-ping-result     & deviceID, code (int), comment                                                                              & \begin{tabular}[c]{@{}l@{}}результат пинга\\ code 0: пришёл обратный ответ (понг)\\ code 1: устройство не найдено \\ code 2: ошибка пингования\\ code 3: неправильный тип устройства (тип устройства не может выполнить эту операцию)\\ code 4: не удалось распарсить JSON-сообщение;\end{tabular}                                                                                          & сервер   \\
ms-get-address     & deviceID                                                                                                   & запрос на получения адреса                                                                                                                                                                                                                                                                                                                                                                  & клиент   \\
ms-address         & deviceID, code (int), comment                                                                              & \begin{tabular}[c]{@{}l@{}}получение адреса МС-ТЮК клиентом\\ \\ code 0: получен адрес, в comment содержится адрес\\ code 1: устройство не найден\\ code 2: получена ошибка при попытке узнать адрес, в comment содержится текст ошибки\\ code 3: неправильный тип устройства (тип устройства не может выполнить эту операцию)\\ code 4: не удалось распарсить JSON-сообщение;\end{tabular} & сервер   \\
