...
Для того, чтобы добавить шаблон нового устройства для обнаружения загрузчиком, нужно открыть файл [device_list.JSON](https://github.com/Roundabout1/lapki-flasher/blob/main/src/device_list.JSON) и дополнить его описанием нового устройства.

Описание должно содержать следующие поля:
- ID: уникальный идентификатор шаблона (это именно идентификатор описания, а не самого устройства, оно назначается самим разработчиком)
...
- vendorIDs: возмонжые значения vendorID, требуется для обнаружения устройства (можно найти через базу данных: https://devicehunt.com/)
- controller: контроллер устройства, требуется для avrdude
- programmer: программатор устройства, требуется для avrdude
- bootloaderID: уникальный идентификатор шаблона bootloader (см. настройка bootloader) версии устройства, если отсутствует, то значение должно быть равным -1. 

### Настройка bootloader
Если устройство прошивается через bootloader (как arduino micro), то это значит, что оно состоит из двух устройств, каждому необходимо представить своё описание, при основное устройство должно ссылаться на bootloader версию, а сам bootloader, не должен ссылаться на что-либо (см. описания Arduino Micro и Arduino Micro (booltloader) для примера в файле со списком устройств).
...
|----------------------|-----------|-----------------------------------------------------------------------------------------------|
| flash-wrong-id       |           | устройства с таким ID отсутствует в списке                                                    |
